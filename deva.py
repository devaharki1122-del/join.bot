...
async def check_force_join(client, user_id):
    for ch in FORCE_CHANNELS:
        try:
            ...
            if m.status not in ("member", "administrator", "creator", "owner"):
                return False
        except:
            return False
    return True
...
async def start(client, message):
    ...
    await message.reply(
        ...
        "🤖 من بوتێکم بۆ force join گرووپەکان\n\n"
...
async def panel(client, message):
    try:
        ...
        if m.status not in ("administrator", "creator", "owner"):
...
async def callbacks(client, query):
    ...
    if m.status not in ("administrator", "creator", "owner"):
        return await query.answer("❌ تۆ admin نیت", show_alert=True)
